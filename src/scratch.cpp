...
public:
  arma::mat U, D, V;
  SparseMatrix A;
  CitationEstimate(const SparseMatrix&, const arma::mat&, const arma::rowvec&, const arma::mat&);
  arma::vec left_multiply(const arma::vec&);
  arma::vec right_multiply(const arma::vec&);
  double p_omega_z();
};

CitationEstimate::CitationEstimate(
  const SparseMatrix& A1,
  const arma::mat& U1,
  const arma::rowvec& d1,
  const arma::mat& V1) {
  U = U1;
...
arma::vec CitationEstimate::left_multiply(const arma::vec& x) {
...
arma::vec CitationEstimate::right_multiply(const arma::vec& x) {
  ...
  for (size_t col = 0; col < A.cols(); ++col) {
    for (SparseMatrix::InnerIterator it(A, col); it; ++it) {
      ...
      if (i >= col) {
        p3(col) += x(i) * dot(S.row(i), V.row(col));
      }
    }
  }
...
// [[Rcpp::export]]
Rcpp::XPtr<CitationEstimate> makeCitationEstimate(
